...
VtuComponent({
  ...
  ready: function () {
    ...
    if (
      !this.data.value &&
      (this.data.current || this.data.current == 0) &&
      this.data.tabList[this.data.current]
    ) {
      this.setData({
  ...
  methods: {
    ...
    changeContent: function () {
      let self = this;
      this.getRelationNodes("../tab/vtu-tab", (nodes) => {
        nodes.map((v, i) => {
      ...
      self
        ...
        .boundingClientRect(function (rects) {
          ...
          if (rect) {
            ...
            left +=
              (rect.width - width) / 2 +
              (rects[0].left > 0 ? rects[0].left - self.data.tabLeft : 0);
            var line = tt.createAnimation({});
            line.width(width).left(left).step({
              duration: 300,
            });
            self.setData({
